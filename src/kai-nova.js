...
async function fetchAvailablePowers() {
    const base = cfg.POWERS_INDEX_BASE;
    const indexMd = await httpsGet(base + "/INDEX.md");
    if (!indexMd) { log("Could not fetch powers index from " + base + "."); return; }
    ...
    sections.push("## Creating new powers\n" +
        ...
        "Follow the structure at " + base + "/STRUCTURE.md\n" +
    ...
    log("Available powers fetched from " + base + " (" + names.length + " powers).");
}
